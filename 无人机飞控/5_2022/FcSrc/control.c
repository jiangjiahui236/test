...
int square_wave(int amplitude)//阶跃函数，周期10s
{
	...
	if(time_cnt_ms/5000%2==1)out=-amplitude;
	return out;
}

int triangualr_wave(int amplitude)//三角函数，周期6s
{
	int out;
	if(time_cnt_ms/3000%2==0)out=time_cnt_ms%3000/3000.0*amplitude;
	if(time_cnt_ms/3000%2==1)out=(1-time_cnt_ms%3000/3000.0)*amplitude;
	return out;
}
