	...
	//判断第6通道拨杆位置 800<CH_6<1200，右拨杆向上
		...
		if(one_key_mission_f==1)
		{
			switch(mission_step)
			{
				...
				//OpenMV定位起点
//				case 5:{to_OpenMV_data_0=1;mode_select(3);mission_step++;}break;
//				case 6:{if(OpenMV_data_1>-5&&OpenMV_data_1<5&&OpenMV_data_2>-5&&OpenMV_data_2<5) cnt++;else cnt=0;
//					if(cnt>=25){mission_step++;cnt=0;}}break;
				//重新建图
				case 5:{KEY_1=1;User_Task_Delay(500);}break;
				case 6:{KEY_1=0;if(radar_wait_time<=10)mission_step++;}break;
				case 7:{mission_step=10;}break;////////////////////////////////
...
//					dis_y_zore_fix=dis_y;
				...
				case 12:{
					if(dis_x_target-dis_x_fix>-5&&dis_x_target-dis_x_fix<5&&dis_y_target-dis_y_fix>-5&&dis_y_target-dis_y_fix<5)cnt++;else cnt=0;//到达目标
					if(cnt>=25){mission_step++;cnt=0;}
				}break;
				case 13:{
					if(dis_target_ref<8){dis_target_ref++;mission_step=11;}//更新目标编号
					else mission_step=19;//到达目标8后
				}break;
