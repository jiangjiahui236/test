...
void PID(void)//遥控输入值的PID运算
{
	...
	if(test_flag)				keep_dis_x(dis_x_target,dis_x_fix,50);
	if(keep_dis_x_flag)	keep_dis_x(dis_x_target,dis_x_fix,50);
	if(keep_OpenMV_data_1)keep_dis_x(0,OpenMV_data_1,10);
	
	if(test_flag)				keep_dis_y(dis_y_target,dis_y_fix,50);
	if(keep_dis_y_flag)	keep_dis_y(dis_y_target,dis_y_fix,50);
	if(keep_OpenMV_data_2)keep_dis_y(0,OpenMV_data_2,10);

	if(test_flag)				keep_yaw(yaw_target,yaw_fix);
