...
{
	int32_t error,KP_out,KI_out,KD_out;
	static int32_t dis_x_intrgrate,error_last;
	...
	error=Limit_max_int(error,10);
	dis_x_intrgrate += error;
	KI_out=Limit_max_int(KI_out,intrgrate_max);
	
	my_give_vel_x= KP_out+KI_out+KD_out;
...
{
	int32_t error,KP_out,KI_out,KD_out;
	static int32_t dis_y_intrgrate,error_last;
	...
	error=Limit_max_int(error,10);
	dis_y_intrgrate += error;
	KI_out=Limit_max_int(KI_out,intrgrate_max);
	
	my_give_vel_y= KP_out+KI_out+KD_out;
...
void PID(void)//遥控输入值的PID运算
{
	...
	if(keep_dis_x_flag)	  keep_dis_x(KP_xy,KD_xy,dis_x_target,dis_x_fix,30);
	if(keep_radar_x_flag) keep_dis_x(KP_radar,KD_radar,dis_x_target,radar_data_1,30);
	if(keep_OpenMV_data_1)keep_dis_x(KP_MV,KD_MV,0,OpenMV_data_1,20);
	
	if(keep_dis_y_flag)		keep_dis_y(KP_xy,KD_xy,dis_y_target,dis_y_fix,30);
	if(keep_radar_y_flag) keep_dis_y(KP_radar,KD_radar,dis_y_target,radar_data_2,30);
