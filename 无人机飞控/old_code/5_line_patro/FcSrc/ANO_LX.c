...
//		}
		...
		if(my_code_control_flag==1)
		{
			...
			rt_tar.st_data.yaw_dps = -my_give_vel_yaw;  //航向转动角速度，度每秒，逆时针为正
			rt_tar.st_data.vel_x = my_give_vel_x;    //头向速度，厘米每秒
			rt_tar.st_data.vel_y = my_give_vel_y;    //左向速度，厘米每秒
			rt_tar.st_data.vel_z = my_give_vel_z;	 //天向速度，厘米每秒
		}
		//########################################################################
		//=====
