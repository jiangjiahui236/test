...
u8 rise_Move(u16 dis_cm, u16 vel_cmps)//上升
{
	...
	if (dt.wait_ck == 0) //没有其他等待校验的CMD时才发送本CMD
	{
		...
		dt.cmd_send.CMD[1] = 0X01;
		//
		dt.cmd_send.CMD[2] = BYTE0(dis_cm);
		dt.cmd_send.CMD[3] = BYTE1(dis_cm);
		dt.cmd_send.CMD[4] = BYTE0(vel_cmps);
		dt.cmd_send.CMD[5] = BYTE1(vel_cmps);
		//
		CMD_Send(0xff, &dt.cmd_send);
		return 1;
	}
...
u8 right_Move(u16 angle, u16 angle_vel)//右旋
{
	...
	if (dt.wait_ck == 0) //没有其他等待校验的CMD时才发送本CMD
	{
		...
		dt.cmd_send.CMD[4] = BYTE0(angle_vel);
		dt.cmd_send.CMD[5] = BYTE1(angle_vel);
		//
		CMD_Send(0xff, &dt.cmd_send);
		return 1;
	}
