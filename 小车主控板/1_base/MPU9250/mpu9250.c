...
{
	if(mpu_mpl_get_data(&pose.roll,&pose.pitch,&pose.yaw)==0)
  {
		if((pose.last_yaw>90) && (pose.yaw<-90))		
	{
		pose.round_flag++;
	}
		if((pose.last_yaw<-90) && (pose.yaw>90))			pose.round_flag--;
		pose.last_yaw = pose.yaw;
		yaw = (int)(pose.yaw) + (int)(pose.round_flag)*360;
	}
}
